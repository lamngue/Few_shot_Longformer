...
class TransformerMarco(pl.LightningModule):
    ...
    def configure_optimizers(self):
        optimizer = AdamW(self.parameters(), lr=self.hparams_ns.lr,
                betas=(0.9, 0.999), weight_decay=0.01, correct_bias=False)
        scheduler = {'scheduler': get_linear_schedule_with_warmup(optimizer,
                            self.hparams_ns.num_warmup_steps,
                            self.hparams_ns.num_training_steps),
                    'interval': 'step',
                    'name': 'linear_with_warmup'}
        return [optimizer], [scheduler]

    def train_dataloader(self):
        dataset = self.DatasetClass(data_dir=self.hparams_ns.data_dir,
                        mode='train',
                        tokenizer=self.tokenizer,
                        max_seq_len=self.hparams_ns.max_seq_len,
                        n_tr_qrs = self.hparams_ns.n_tr_qrs,
                        args=self.hparams_ns
                        )
        ...
        self.train_dataloader_object = DataLoader(
                                    dataset, batch_size=self.hparams_ns.data_loader_bs,
                                    shuffle=(sampler is None),
                                    num_workers=self.hparams_ns.num_workers, sampler=sampler,
    ...
    def val_dataloader(self):
        ...
        self.val_dataloader_object = DataLoader(
                                    dataset, batch_size=self.hparams_ns.val_data_loader_bs,
                                    shuffle=False,
                                    num_workers=self.hparams_ns.num_workers, sampler=sampler,
                                    collate_fn=TransformerMarco.collate_fn
                                    )
    ...
    def test_epoch_end(self, outputs):
        ...
        outputs['idxs'] --> indexes in Dataset to connect with scores
        """
        mrr10 = self._get_mrr_score(outputs,k=10, mode='test')
        mrr100 = self._get_mrr_score(outputs,k=100, mode='test')
        print(mrr10)
        print(mrr100)
    ...
    def _store_trec_output(self, outputs):
        ...
        df['Q0'] = 'Q0'
        df['run_name'] = self.hparams_ns.run_name
        df['rank'] = df.groupby('qid')['prob'].rank(ascending=False)
        #df.rank = df.rank.astype(int)
        df = df[['qid', 'Q0', 'did', 'rank', 'prob', 'run_name']]
        df.to_csv(f'{self.hparams_ns.res_dir}{self.hparams_ns.run_name}.tsv', sep=' ', header=False, index=False)
        return